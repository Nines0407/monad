...
program
  ...
  .action(async (options) => {
    ...
    const categories = options.allowedCategories.split(',').map((c: string) => c.trim());
    ...
    console.log(`Budget: ${formatAmount(options.budget, options.currency)}`);
    console.log(`Daily limit: ${formatAmount(options.dailyLimit, options.currency)}`);
    console.log(`Require approval threshold: ${formatAmount(options.requireApproval, options.currency)}`);
...
program
  ...
  .action(async (options) => {
    ...
    if (!validateAddress(options.recipient)) {
    ...
    console.log(`  Amount: ${formatAmount(options.amount, options.currency)}`);
    console.log(`  Recipient: ${options.recipient}`);
    console.log(`  Category: ${options.category}`);
    console.log(`  Reason: ${options.reason}`);
    if (options.agentId) console.log(`  Agent ID: ${options.agentId}`);
    if (options.taskId) console.log(`  Task ID: ${options.taskId}`);
...
program
  ...
  .action(async (options) => {
    ...
    if (options.agentId) filters.push(`agent: ${options.agentId}`);
    if (options.userId) filters.push(`user: ${options.userId}`);
  ...
  .action(async (options) => {
    ...
    if (options.agentId) {
      ...
      if (agent) {
        ...
        console.log('\nBudget:');
        console.log(`  Total: ${formatAmount(agent.budget.total.toString(), agent.budget.currency)}`);
        console.log(`  Spent: ${formatAmount(agent.budget.spent.toString(), agent.budget.currency)}`);
        console.log(`  Remaining: ${formatAmount(agent.budget.remaining.toString(), agent.budget.currency)}`);
        console.log(`  Daily Limit: ${formatAmount(agent.budget.dailyLimit.toString(), agent.budget.currency)}`);
        console.log(`  Daily Spent: ${formatAmount(agent.budget.dailySpent.toString(), agent.budget.currency)}`);
        ...
        if (options.verbose) {
          ...
          console.log(`  Max Daily Amount: ${formatAmount(agent.permissions.maxDailyAmount.toString(), agent.budget.currency)}`);
          console.log(`  Allowed Categories: ${agent.permissions.allowedCategories.join(', ')}`);
          console.log(`  Approval Threshold: ${formatAmount(agent.permissions.requireApprovalThreshold.toString(), agent.budget.currency)}`);
        }
      } else {
      ...
      Object.keys(auth).forEach(agentId => {
        ...
        console.log(`  - ${agentId}: ${formatAmount(agent.budget.remaining.toString(), agent.budget.currency)} remaining`);
      });
    }
